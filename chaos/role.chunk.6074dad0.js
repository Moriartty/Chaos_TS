...
var RoleList = /** @class */ (function (_super) {
    ...
    RoleList.prototype.render = function () {
        ...
        return (React.createElement("div", null, list.length ? (React.createElement(antd_1.Menu, { onClick: this.handleClick }, list.map(function (o) {
            _this.roleObj[o.id] = o;
            return (React.createElement(antd_1.Menu.Item, { key: o.id },
...
var RoleMenu = /** @class */ (function (_super) {
    ...
    RoleMenu.prototype.handleCheckChange = function (e) {
        var roleAuth = this.props.roleAuth.concat();
    ...
    RoleMenu.prototype.renderItem = function (item) {
        ...
        var list = item.list;
        var roleMenuIds = this.props.roleAuth;
        return (React.createElement("li", { key: item.index },
            React.createElement("div", { className: "no-select menu-name" },
                ...
                React.createElement("label", { className: "cursor-pointer" },
                    React.createElement("input", { type: "checkbox", checked: index_1.hasChildInTree(item, roleMenuIds), value: item.id, onChange: this.handleCheckChange.bind(this) }),
    ...
    RoleMenu.prototype.render = function () {
        var _this = this;
        var _a = this.props, operations = _a.operations, menuTree = _a.menuTree, roleInfo = _a.roleInfo, roleAuth = _a.roleAuth, onSave = _a.onSave;
        return (React.createElement("div", { className: "role-menu" },
            React.createElement("ul", { className: "unstyled" }, menuTree.map(function (o, i) {
                o.index = i;
                return _this.renderItem(o);
            })),
            operations.indexOf('role_operation_modify') >= 0 && (React.createElement(antd_1.Button, { type: "primary", icon: "save", className: "save", onClick: onSave.bind(this, roleInfo.id, roleAuth) }, "\u4FDD\u5B58"))));
    };
    return RoleMenu;
}(React.Component));
var RoleMenuComp = react_redux_1.connect(function (state) {
    var operations = state.app.menuObj['systemConfig/role'].functions;
    var _a = state.role, menuTree = _a.menuTree, roleInfo = _a.roleInfo, roleAuth = _a.roleAuth;
    return { operations: operations, menuTree: menuTree, roleInfo: roleInfo, roleAuth: roleAuth };
}, function (dispatch) { return ({
    /**
     * 改变复选框
     * @param roleAuth
     */
    onCheckChange: function (roleAuth) {
        console.log('roleAuth', roleAuth);
        dispatch({ type: 'ROLE_MENU_CHK_CHANGE', roleAuth: roleAuth });
    },
    /**
     * 保存
     * @param roleId
     * @param roleAuth
     */
    onSave: function (roleId, roleAuth) {
        // 格式转化
        var obj = {}; // {6:[], 9:["CREATE","UPDATE"]}
        roleAuth.forEach(function (id) {
            // eslint-disable-next-line
            if (id != '0') { // 排除手动添加的
                var index = id.indexOf('_');
                if (~index) {
                    // 操作权限类型
                    var menuId = id.slice(0, index);
                    if (!obj[menuId]) {
                        obj[menuId] = [];
                    }
        ...
        // 对象转数组
        var data = [];
        for (var menuId in obj) {
            data.push({
                menuId: menuId,
                functions: obj[menuId].length ? obj[menuId] : undefined
            });
        }
        dispatch(role_1.default.updateRoleMenu(roleId, data)).then(function () {
...
var Role = /** @class */ (function (_super) {
    ...
    Role.prototype.render = function () {
        var _a = this.props, operations = _a.operations, role = _a.roleInfo, onAdd = _a.onAdd, onEdit = _a.onEdit, onDelete = _a.onDelete;
        return (React.createElement("div", { className: "page-component" },
            ...
            role.id ? (React.createElement("div", { className: "role-right" },
                ...
                React.createElement(RoleMenu_1.default, null))) : (React.createElement("div", { className: "role-right" },
...
var RoleComp = react_redux_1.connect(function (state) {
    var operations = state.app.menuObj['systemConfig/role'].functions;
    var roleInfo = state.role.roleInfo;
    return { operations: operations, roleInfo: roleInfo };
}, function (dispatch) { return ({
    init: function () {
        dispatch(role_1.default.loadList());
